...
router.get("/usuario/name", async (req, res) => {
  res.status(202).send("Este es el perfil de : Coquitos");
}); // ❓❓❓❓❓
...
router.get("/products", async (req, res) => {
  ...
  if (nombre) {
    let productName = await allprod.filter((el) =>
      el.title.toLowerCase().includes(nombre.toLowerCase())
    );
    productName.length
      ? res.status(200).send(productName.slice(0, 15))
...
router.get("/filterByCategory", async (req, res) => {
  try {
    ...
    const products = await filterByCategory(categorias);
